...
with(shs_df, boxplot(tst_hr ~ shs_di))
with(shs_df, boxplot(mpsd ~ shs_di))
with(shs_df, boxplot(sdtst ~ shs_di))
table(shs_df$shs_di)
...
shs_sens_df$shs_di <- as.numeric(shs_sens_df$shs_di)
cluster_df <- na.omit(shs_sens_df)

m.out = matchit(shs_di ~ age5c + female + race + 
                modvig_pa + smoke + income_tertile + 
                degree_attain + married + aehi_10 + 
                  prev_cvd+ prev_cancer + prev_copd +
                  bmi, 
                data = cluster_df, 
                method= "nearest",
                ratio = 1,
...
# m.out_splines = matchit(shs_di ~ rcs(age5c, 4) + female + race + 
...
m.out_ks = matchit(shs_di ~ age5c + female + race + 
                          modvig_pa + smoke + income_tertile + 
...
fit_matched <- coxph(Surv(dthtt-slpexam1, death) ~ shs_di, data = m.data1,
                             weights = weights, cluster = subclass)
summary(fit_matched)
ShowRegTable(fit_matched)

fit_matched_ks <- coxph(Surv(dthtt-slpexam1, death) ~ shs_di, data = m.data1_ks,
                                weights = weights, cluster = subclass)
...
fit_matched_cov <- coxph(Surv(dthtt-slpexam1, death) ~ shs_di + female +
...
fit_unmatched <- coxph(Surv(dthtt-slpexam1, death) ~ shs_di + age5c + female + race + income_tertile + 
...
fit_unmatched_sens <- coxph(Surv(dthtt-slpexam1, death) ~ shs_di + age5c + race + income_tertile + 
...
summary(fit_unmatched_tst_sens )
  ...
  # Model on matched data
fit_matched_withcov <- coxph(Surv(dthtt-slpexam1, death) ~ shs_di, data = m.data1,
                             weights = weights, cluster = subclass)
summary(fit_matched_withcov)
ShowRegTable(fit_matched_withcov)

fit_matched_withcov_ks <- coxph(Surv(dthtt-slpexam1, death) ~ shs_di, data = m.data1_ks,
                             weights = weights, cluster = subclass)
summary(fit_matched_withcov_ks)
ShowRegTable(fit_matched_withcov_ks)

# Adjusting for PScore
fit_matched_withPScore <- coxph(Surv(dthtt-slpexam1, death) ~ shs_di + distance, data = m.data1,
                                weights = weights, cluster = subclass)
summary(fit_matched_withPScore)

# Model on matched data not including covariates used in PScore
fit_matched_withoutcov <- coxph(Surv(dthtt-slpexam1, death) ~ shs_di, data = m.data1, robust = TRUE,
                                weights = weights, cluster = subclass)
summary(fit_matched_withoutcov)

# Model on matched data including PScore as covariate
fit_matched_with_pscore <- coxph(Surv(dthtt-slpexam1, death) ~ shs_di + distance, data = m.data1, robust = TRUE,
                                 weights = weights, cluster = subclass)
summary(fit_matched_with_pscore)

# Model on iptw
fit_iptw <- coxph(Surv(dthtt-slpexam1, death) ~ shs_di, data = m.data1,
                       weights = iptw, model = T)
summary(fit_iptw)

# Model on bmi/ahi matched data including covariates used in PScore
fit_matched_withcov <- coxph(Surv(dthtt-slpexam1, death) ~ shs_di + age5c + race + income_tertile + 
                               degree_attain + married + aehi_10 + smoke + modvig_pa, data = m.data1_ahi_bmi,
                             weights = weights, cluster = subclass)
summary(fit_matched_withcov)

# kitchen sink pscore model
fit_matched_ks <- coxph(Surv(dthtt-slpexam1, death) ~ shs_di, 
                                data = m.data1_ks,
                                weights = weights, cluster = subclass)
summary(fit_matched_ks)
...
# Generate IPTW weights
m.data1_ks$iptw <- ifelse(m.data1_ks$shs_di == "Favorable sleep",
                       1/m.data1_ks$distance,
                       1/(1-m.data1_ks$distance))
favstats(m.data1_ks$iptw)

# kitchen sink pscore model
fit_matched_ks_iptw <- coxph(Surv(dthtt-slpexam1, death) ~ shs_di, 
                        data = m.data1_ks,
                        weights = iptw)
...
boot_res <- censboot(data = m.data1_ks,
                     boot_fun, R = 999,
                     formula = Surv(dthtt-slpexam1, death) ~ shs_di + age5c + race + income_tertile + 
                       degree_attain + married + aehi_10 + smoke + modvig_pa,
                     distr = "loglogistic"
)
