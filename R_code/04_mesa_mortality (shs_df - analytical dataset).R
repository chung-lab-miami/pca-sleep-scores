...
mesa_subset_df <- mesa_df %>% 
  dplyr::select(-pc1_scaled.y) %>%
  mutate(pc1_scaled_test = pc1_scaled.x) %>% 
  dplyr::select(-pc1_scaled.x) %>%
  mutate(pc1_scaled = pc1_scaled_test) %>%
...
shs_df_1 <- merge(mesa_subset_df, aehi_df, by = "idno") # shs_df, n=1,728
shs_df <- merge(shs_df_1, mesa_all_events, by = "idno") # shs_df, n=1,726
